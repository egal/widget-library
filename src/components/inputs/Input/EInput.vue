<template>
  ...
  >
    ...
    >
      <div class="chips-container">
        ...
        >
          ...
          <b-icon
              icon="x-lg"
              class="chip--close"
              @click.stop="$emit('delete-option', selected)"
          />
        </div>
      </div>
      ...
      <div
          class="arrow-icons"
          v-if="type === 'number' && mergedData.showArrows"
      >
        <b-icon
    ...
    <p
        :class="[
        'helper-text',
        (errorMessage || mergedData.error) && mergedData.showError
          ? 'helper-text--error'
          : '',
      ]"
...
import {defaultValidators} from "@/components/table/helpers/validators";
export default {
  name: 'EInput',
  components: {
    ClearButton,
    ...
    getStyleVars() {
      return {
        ...
        '--background-disabled-color':
            this.styleConfig?.backgroundDisabledColor || '#F7FAFC',
        '--focus-border-color': this.styleConfig?.focusBorderColor || '#76ACFB',
        '--filled-background-color':
            this.styleConfig?.filledBackgroundColor || '#DEEBFC',
        '--filled-font-color': this.styleConfig?.filledFontColor || '#3385ff',
        '--search-background-color':
            this.styleConfig?.searchBackgroundColor || '#f7fafc',
  ...
  methods: {
    ...
    inputHandler() {
      ...
      if (this.mergedData.validators?.length) {
        if(this.table) {
          Object.entries(defaultValidators).forEach(entry => {
            const [key, value] = entry;
            this.mergedData.validators.forEach(validator => {
              console.log(key, validator)
              if(key === validator) {
                this.tableValidators.push(defaultValidators[key])
              }
            })
          });
          this.errorMessage = validate(this.tableValidators, this.newValue)
        } else {
          this.errorMessage = validate(this.mergedData.validators, this.newValue)
        }
        if (this.table) {
          this.$emit('update:modelValue', {name: 'input', value: this.errorMessage})
        } else {
          this.$emit('error', this.errorMessage)
        }
      }
      if (this.table) {
        this.$emit('update:modelValue', {name: 'input', value: this.newValue})
      } else {
