...
|  ~~``localeOptions``~~   |       ~~Object~~    | ~~Объект~~   <br/> **(решено с дизайнерами убрать кастомизацию формата даты)**                                  |  ~~Опции для форматирования даты (те же что и для ``Date.prototype.toLocaleString()``), чтобы кастомизировать отображение даты и времени в инпуте~~          |
